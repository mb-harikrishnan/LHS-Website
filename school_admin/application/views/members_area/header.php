...
/* ─────────────────────────────────────────────
   ...
   Tables involved:
     menus (menu_id, parent_menu_id, menu_name, display_name,
            menu_link, display_order, status, group)
   ...
   Logic:
     ...
     - Split top-level parents into two sidebar sections by `group`:
         group = 1 -> "Main Menu"
         group = 2 -> "Teachers"
   ───────────────────────────────────────────── */
...
// Split top-level parents into sidebar sections by group_id.
// group 1 -> "Main Menu", group 2 -> "Teachers".
$menuSections = [
...
foreach ($menuTree as $parent) {
    ...
    if (!isset($menuSections[$gid])) {
        // fallback bucket for any unexpected group value
...
<body>
  ...
  <div class="layout">
    ...
    <aside class="sidebar" id="sidebar">
      ...
      <!-- Navigation (dynamic, filtered by role permissions, grouped) -->
      <nav class="sb-nav">
        ...
        <?php foreach ($menuSections as $gid => $section):
            if (empty($section['items'])) continue;
        ?>
          <div class="sb-section-label"><?php echo htmlspecialchars($section['label']); ?></div>
